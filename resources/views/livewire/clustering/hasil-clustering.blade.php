    ...
    <div class="row">
        <div class="col-12">
            ...
            @if (!empty($jumlahPerCluster))
                ...
                <hr> {{-- Tambahkan garis setelah tabel --}}
            @endif

            {{-- Tabel IKU per Klaster --}}
                    ...
                    @forelse ($paginatedData as $index => $row)
                        <tr>
                            <td class="text-center">
                                {{ $loop->iteration + ($paginatedData->currentPage() - 1) * $paginatedData->perPage() }}
                            </td>
                            <td>{{ $row['id_usulan'] }}</td>
                            <td>{{ $row['iku'] }}</td>
            ...
            <div class="d-flex justify-content-between align-items-center mt-3">
                ...
                <button wire:click="$emitUp('lanjutKeVisualisasi')" class="btn btn-primary">
